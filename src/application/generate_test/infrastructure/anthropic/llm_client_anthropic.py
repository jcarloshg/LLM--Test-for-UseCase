...
class LLMClientAnthropic(LlmClient):
    ...
    def generate(self, prompt: str, system_prompt: str = "") -> ILlmClientResponse:
        ...
        Raises:
            LLMClientException: If the Anthropic API call fails or returns an error
        """
        start = time.time()
        ...
        try:
            ...
            # Re-raise LLMClientException as-is
            raise
        except Exception as e:
            # Wrap any other exceptions in LLMClientException
            raise LLMClientException(
                message=f"Failed to generate text from Anthropic: {str(e)}",
                model=self.config.model,
                provider="anthropic",
                original_error=e
            )
